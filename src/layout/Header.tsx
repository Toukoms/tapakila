...
function Header() {
  ...
  return (
    <header className="relative w-full flex justify-between items-center px-12 py-8 bg-base-200">
      ...
      <nav className="sm:w-auto! sm:static! sm:flex-row! sm:bg-transparent! sm:py-0! peer-has-checked:flex peer-has-checked:flex-col peer-has-checked:absolute peer-has-checked:top-full peer-has-checked:left-0 peer-has-checked:py-10 peer-has-checked:w-full peer-has-checked:h-auto peer-has-checked:scale-100 transition-transform duration-700 peer-has-checked:bg-base-300 items-center gap-6 hidden h-0 scale-y-0 sm:scale-100 sm:h-auto sm:flex">
        {navLinks.map((l) => (
          <a
